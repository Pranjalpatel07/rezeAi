...
export default function App() {
  ...
  return (
    <div className="h-screen flex items-center justify-center bg-bg text-text">
      <div className="w-full max-w-md h-[90vh] bg-panel rounded-2xl flex flex-col overflow-hidden">

        {/* Header */}
        <div className="flex items-center gap-3 px-5 py-4 bg-indigo-950 bg-accent/10">
          <div className="w-10 h-10 bg-accent bg-violet-400 rounded-full flex items-center justify-center font-bold text-black">
            R
          </div>
          <div>
            <p className="font-semibold text-white">Reze</p>
            <p className="text-xs text-accent text-violet-500">Online • Cafe Crossroads</p>
          </div>
        </div>
          ...
          {messages.map((m, i) => (
            <div
              ...
              className={`max-w-[85%] px-4 py-3 text-sm rounded-2xl
                ${m.role === "user"
                  ? "ml-44  bg-[#42429b] text-white rounded-br-sm"
        ...
        {/* Input */}
        <div className="flex gap-2 p-4 bg-[#161636] border-t border-white/10">
          <input
            value={input}
            onChange={e => setInput(e.target.value)}
            onKeyDown={e => e.key === "Enter" && sendMessage()}
            placeholder="Whisper something to Reze..."
            className="flex-1 px-4 py-2 rounded-full bg-white border border-white/20 outline-none text-sm"
          />
          <button
            onClick={sendMessage}
            className="px-5 rounded-full bg-accent bg-[#6565a8] text-white font-semibold"
          >
            Send
          </button>
        </div>
